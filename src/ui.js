(function () {
  ...
  const PALETTE = ["#0f172a", "#0ea5e9", "#d97706", "#7c3aed", "#db2777", "#0d9488", "#65a30d"];
  const DEFAULTS = {
  ...
  const proxyLine = {
    ...
    afterDraw(c) {
      ...
      ctx.setLineDash([]); ctx.fillStyle = "#64748b"; ctx.font = "10px sans-serif";
  ...
  function renderChart(type) {
    ...
    if (type === "equity" || type === "drawdown") {
      ...
      ds.push(mk("Strategy", master, cur.eq, "#0f172a", 2.2));
    ...
    chart = new Chart($("mainChart"), {
      ...
      options: {
        ...
        scales: {
          x: { type: "category", ticks: { maxTicksLimit: 10, autoSkip: true, color: "#64748b", font: { size: 10 } }, grid: { display: false } },
          y: isAlloc ? { stacked: true, min: 0, max: 100, ticks: { callback: v => v + "%", color: "#64748b", font: { size: 10 } }, grid: { color: "#eef1f5" } }
            : isDD ? { ticks: { callback: v => (v * 100).toFixed(0) + "%", color: "#64748b", font: { size: 10 } }, grid: { color: "#eef1f5" } }
            : { type: $("logScale").checked ? "logarithmic" : "linear", ticks: { callback: v => moneyShort(v), color: "#64748b", font: { size: 10 } }, grid: { color: "#eef1f5" } }
        },
        plugins: {
          legend: { position: "bottom", labels: { boxWidth: 12, boxHeight: 12, font: { size: 11 }, color: "#334155", usePointStyle: true } },
  ...
  function renderAnnual() {
    ...
    const sets = [bar("Strategy", cur.m, "#0f172a")];
    ...
    chart = new Chart($("mainChart"), {
      ...
      options: { responsive: true, maintainAspectRatio: false, animation: false,
        scales: { x: { grid: { display: false }, ticks: { color: "#64748b", font: { size: 10 } } }, y: { ticks: { callback: v => v + "%", color: "#64748b", font: { size: 10 } }, grid: { color: "#eef1f5" } } },
        plugins: { legend: { position: "bottom", labels: { boxWidth: 12, font: { size: 11 }, color: "#334155", usePointStyle: true } }, tooltip: { callbacks: { label: c => `${c.dataset.label}: ${c.parsed.y == null ? "n/a" : c.parsed.y.toFixed(1) + "%"}` } } } }
    });
  ...
  function persist() { try { localStorage.setItem("lrs_state_v4", JSON.stringify({ params: readParams(), versions, compare, editingVersionId })); } catch (e) {} }
  function restore() {
    try {
      const s = JSON.parse(localStorage.getItem("lrs_state_v4"));
      if (s && s.params) { params = { ...DEFAULTS, ...s.params }; prevAnchor = params.anchorKey; versions = s.versions || []; compare = s.compare || compare; editingVersionId = s.editingVersionId || null; }
    } catch (e) {}
  }
  function exportConfig() {
    const blob = new Blob([JSON.stringify(readParams(), null, 2)], { type: "application/json" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "strategy_config.json"; a.click();
    setNL("Exported strategy_config.json — commit it to your repo so the alert workflow uses these exact parameters.");
  }
...
(was TQQQ 70% | JEPQ 30%)</pre>
      <button class="btn" id="btnExportCfg2">Export config for the alert script</button></div></div>`;
    $("btnExportCfg2").onclick = exportConfig;
  }
  function init() {
    ...
    restore(); buildControls(); updateSummary(); renderAlerts();
    $("btnRun").onclick = run;
